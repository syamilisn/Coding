...
static int value1 = 0, value2 = 0, count = 0;
...
static irqreturn_t ir_sensor_irq_handler(int irq, void * dev_id) {
    ...
    value1 = gpio_get_value(SENSOR1_PIN);
    value2 = gpio_get_value(SENSOR2_PIN);

    if (value1 == 1) {
        pr_info("Detected at 1st sensor person entering the room\n");
        pr_info("Setting the 2nd ir sensor low \n");
        gpio_set_value(SENSOR2_PIN, 0);
        pr_info("Setting ir2 to 0");
        count++;
        gpio_set_value(SENSOR2_PIN, 1);
    } else if (value2 == 1) {
        pr_info("Detected at 1st sensor person entering the room\n");
        pr_info("Setting the 2nd IR sensor to low \n");
        gpio_set_value(SENSOR1_PIN, 0);
        count--;
        gpio_set_value(SENSOR1_PIN, 1);
    }

    if (count > 0) {
        pr_info("Count of people the room = %d\n", count);
    } else {
        pr_info("no one is in the roomn");
    }
...
static irqreturn_t interrupt_thread_fn(int irq, void * dev_id) {
    ...
    if(count<=0)
    {
      ssd1306_string_display("There's no one in the room");
      return IRQ_HANDLED;
    }
    else 
    {
...
static int __init etx_driver_init(void) {
    ...
    // Request GPIO pin for IR sensor
    ret = gpio_request(SENSOR2_PIN, "ir_sensor2");
    if (ret < 0) {
        printk(KERN_ERR "Failed to request GPIO %d for IR sensor\n", SENSOR2_PIN);
        return ret;
    }
    ...
    // Request interrupt for the GPIO pin
    ret = request_threaded_irq(gpio_to_irq(SENSOR2_PIN), ir_sensor_irq_handler, interrupt_thread_fn, IRQF_TRIGGER_RISING, "ir_sensor2", NULL);
    if (ret < 0) {
        printk(KERN_ERR "Failed to request interrupt for IR sensor\n");
        gpio_free(SENSOR2_PIN);
        return ret;
    }
...
static void __exit etx_driver_exit(void) {
    ...
    free_irq(gpio_to_irq(SENSOR2_PIN), NULL);
    gpio_free(SENSOR2_PIN);
